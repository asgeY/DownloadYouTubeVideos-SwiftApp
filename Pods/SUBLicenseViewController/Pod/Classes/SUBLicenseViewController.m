...
@property (strong, nonatomic) NSMutableDictionary *licensesHeights;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _licensesHeights = [NSMutableDictionary dictionary];
    }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    NSString *licensesPath = [[NSBundle mainBundle] pathForResource:@"Licenses" ofType:@"plist"];
    ...
    if (!acknowledgementsData) {
        ...
        if (!acknowledgementsPlist) {
            ...
            _licensesHeights[@"Error Loading Licenses"] = @(60.0);
            _licensesHeights[[licensesError localizedDescription]] = @(100.0);
        }
            ...
            for (NSDictionary *specifier in specifiers) {
                ...
                if (licenseTitle && licenseBody) {
                    [loadingLicenses addObject:[SUBLicense licenseWithTitle:licenseTitle body:licenseBody]];
                     CGFloat licenseBodyHeight = [licenseBody boundingRectWithSize:CGSizeMake(self.tableView.frame.size.width - 25.0, INFINITY) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : _licenseBodyFont} context:nil].size.height + 40.0;
                    CGFloat licenseTitleHeight = [licenseTitle boundingRectWithSize:CGSizeMake(self.tableView.frame.size.width - 25.0, INFINITY) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : _licenseTitleFont} context:nil].size.height + 10.0;
                    _licensesHeights[licenseTitle] = @(licenseTitleHeight);
                    _licensesHeights[licenseBody] = @(licenseBodyHeight);
                }
            }
...
- (void)addLicenseWithTitle:(NSString *)title body:(NSString *)body {
    ...
    CGFloat licenseBodyHeight = [body boundingRectWithSize:CGSizeMake(self.tableView.frame.size.width - 25.0, INFINITY) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : _licenseBodyFont} context:nil].size.height + 40.0;
    CGFloat licenseTitleHeight = [title boundingRectWithSize:CGSizeMake(self.tableView.frame.size.width - 25.0, INFINITY) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : _licenseTitleFont} context:nil].size.height + 10.0;
    _licensesHeights[title] = @(licenseTitleHeight);
    _licensesHeights[body] = @(licenseBodyHeight);
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (!_licenses || _licenses.count < section) {
        ...
        return [_licensesHeights[license.licenseTitle] floatValue];
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [_licensesHeights[license.licenseBody] floatValue];
}
